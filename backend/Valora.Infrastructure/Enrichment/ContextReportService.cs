...
public sealed class ContextReportService : IContextReportService
{
    ...
    public async Task<ContextReportDto> BuildAsync(ContextReportRequestDto request, CancellationToken cancellationToken = default)
    {
        ...
        var categoryScores = _scoringCalculator.CalculateCategoryScores(socialMetrics, crimeMetrics, demographicsMetrics, amenityMetrics, environmentMetrics);
        var compositeScore = _scoringCalculator.CalculateCompositeScore(categoryScores);
