...
public class WorkspacePropertyService : IWorkspacePropertyService
{
    ...
    /// A user can request a context report without ever persisting a Property entity beforehand. If the user decides
    /// to save this report to a workspace, we must first ensure a physical representation (the <see cref="Property"/> entity)
    /// exists in our database.
    /// </para>
    /// <para>
    /// <strong>Key Decision:</strong>
    /// The <see cref="ContextReportDto"/> generated by the API does not currently expose a canonical BagId.
    /// As a fallback, this method constructs a pseudo-identifier using the concatenation of PostalCode and DisplayAddress.
    /// If a property with this pseudo-identifier does not exist, a new <see cref="Property"/> is instantiated and persisted
    /// using the normalized metrics from the report.
    /// </para>
    /// </remarks>
    ...
    /// <summary>
    /// Retrieves all comments for a saved property and organizes them into a hierarchical structure.
    /// </summary>
    /// <remarks>
    /// <para>
    /// <strong>Performance & Architecture Decision:</strong>
    /// The comments feature supports nested replies. To build this tree structure, the repository fetches a completely
    /// flat list of comments ordered chronologically in a single SQL query.
    /// This method then performs an in-memory pass to attach child comments to their parents using a dictionary lookup.
    /// </para>
    /// <para>
    /// This approach avoids executing N+1 queries or complex recursive Common Table Expressions (CTEs) at the database level,
    /// significantly improving read performance and reducing database load.
    /// </para>
    /// </remarks>
