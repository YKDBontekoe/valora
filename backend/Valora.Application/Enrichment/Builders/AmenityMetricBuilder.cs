...
public static class AmenityMetricBuilder
{
    public static List<ContextMetricDto> Build(AmenityStatsDto? amenities, NeighborhoodStatsDto? cbs, List<string> warnings)
    {
        ...
        if (amenities != null)
        {
            var proximityScore = AmenityScoringRules.ScoreAmenityProximity(amenities.NearestAmenityDistanceMeters);
            var countScore = AmenityScoringRules.ScoreAmenityCount(
        ...
        if (cbs != null)
        {
            ...
            metrics.Add(new("dist_supermarket", "Dist. to Supermarket", cbs.DistanceToSupermarket, "km", AmenityScoringRules.ScoreProximity(cbs.DistanceToSupermarket, 1.0, 2.5), "CBS StatLine 85618NED"));
            metrics.Add(new("dist_gp", "Dist. to GP", cbs.DistanceToGp, "km", AmenityScoringRules.ScoreProximity(cbs.DistanceToGp, 1.5, 3.0), "CBS StatLine 85618NED"));
            metrics.Add(new("dist_school", "Dist. to School", cbs.DistanceToSchool, "km", AmenityScoringRules.ScoreProximity(cbs.DistanceToSchool, 1.0, 3.0), "CBS StatLine 85618NED"));
