...
public class BatchJobServiceTests
{
    ...
    [Fact]
    public async Task ProcessNextJobAsync_ShouldUpdateExistingNeighborhood()
    {
        var service = CreateService();
        var job = new BatchJob { Type = BatchJobType.CityIngestion, Target = "Amsterdam", Status = BatchJobStatus.Pending };
        _jobRepositoryMock.Setup(x => x.GetNextPendingJobAsync(It.IsAny<CancellationToken>()))
        ...
        var existingNeighborhood = new Neighborhood { Code = "BU001", Name = "Old Name", City = "Amsterdam", Type = "Buurt" };
        _neighborhoodRepositoryMock.Setup(x => x.GetByCodeAsync("BU001", It.IsAny<CancellationToken>()))
            .ReturnsAsync(existingNeighborhood);

        await service.ProcessNextJobAsync();

        Assert.Equal(BatchJobStatus.Completed, job.Status);
        _neighborhoodRepositoryMock.Verify(x => x.AddAsync(It.IsAny<Neighborhood>(), It.IsAny<CancellationToken>()), Times.Never);
        _neighborhoodRepositoryMock.Verify(x => x.UpdateAsync(It.IsAny<Neighborhood>(), It.IsAny<CancellationToken>()), Times.Once);
    }
}
