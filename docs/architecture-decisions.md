...
**Decision:** The Application and Domain layers emit events (e.g., `WorkspaceInviteAcceptedEvent`, `CommentAddedEvent`, `ReportSavedToWorkspaceEvent`, `BatchJobCompletedEvent`, `AiAnalysisCompletedEvent`) rather than orchestrating secondary effects (like notifications) inline.

**Why:**
-   **Separation of Concerns:** Business use-case services like `WorkspaceListingService` are decoupled from side effects like creating notification records.
-   **Extensibility:** Multiple independent event handlers can subscribe to the same domain event without needing to update the primary application service.
-   **Clean Architecture Adherence:** Event dispatching prevents bidirectional dependencies or bloated service constructors.
-   **Maintainability:** Following the single responsibility principle reduces tight coupling, enabling localized logic management.

**Trade-offs:**
-   **Complexity:** Execution paths are less explicit since logic spans multiple files across the `Application` and `Infrastructure` layers instead of one sequential block.
-   **Sync vs Async execution limits:** While the dispatcher can run concurrently, current execution is handled asynchronously but sequentially without complete persistence guarantees out-of-the-box (no transactional outbox pattern implemented).
