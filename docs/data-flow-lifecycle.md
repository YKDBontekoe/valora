...
### The Flow
1.  **Request**: `GET /api/listings/lookup?id={pdokId}`
...
sequenceDiagram
    ...
    Client->>API: GET /api/listings/lookup?id={pdokId}
...
Once a listing exists, it can be "enriched" with Valora's context data (Social, Safety, Amenities). This is often triggered explicitly by an admin.

### The Flow
1.  **Request**: `POST /api/listings/{id}/enrich` (Admin Only)
