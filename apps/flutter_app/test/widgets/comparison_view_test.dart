...
void main() {
  ...
  testWidgets('ComparisonView handles loading/missing individual reports gracefully', (WidgetTester tester) async {
    // Add ID 'C' but report is not in activeReports (simulating fetch pending or failed)
    // We achieve this by adding to comparison via public method but mocking getContextReport to fail or be delayed,
    // but here we just want to test the VIEW's reaction to getReportById returning null.

    // We can't easily put the provider in a state where ID exists but report is null via public API
    // because addToComparison awaits fetchReport.
    // However, if fetchReport fails (as tested in provider tests), the ID is added but report is null.

    // So let's use that scenario.
    // We need a provider instance where fetch fails silently.

    // Create a new provider/service combo for this test to avoid polluting others
    final errorService = _MockApiService();
    // We can't make it fail inside getContextReport because addToComparison catches it?
    // The provider's fetchReport implementation catches the error and notifies listeners.
    // So _activeReports[id] remains null.

    // Let's force failure
    errorService.reports.clear(); // Ensure 'C' is missing

    final errorProvider = ContextReportProvider(
      apiService: errorService,
      historyService: _MockHistoryService(),
    );

    // Trigger add - this will fail to fetch but add ID
    await errorProvider.addToComparison('C', 1000);
    ...
    // Should see loading indicator for the card
