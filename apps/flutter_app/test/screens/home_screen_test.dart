...
void main() {
  ...
  Widget createHomeScreen(ApiService apiService) {
  ...
  group('HomeScreen', () {
    ...
    testWidgets('See All opens Search tab', (WidgetTester tester) async {
      ...
      expect(find.widgetWithText(TextButton, 'See All'), findsNothing);
    });

    testWidgets('Shows error state on listing load error', (
      WidgetTester tester,
    ) async {
      final mockClient = MockClient((request) async {
        if (request.url.toString().contains('health')) {
    ...
    testWidgets('Shows SnackBar on pagination load error', (
      WidgetTester tester,
    ) async {
      int callCount = 0;
      final mockClient = MockClient((request) async {
      ...
      ); // Wait for animations (SlideInItem)
    ...
    testWidgets('Shows scrape button when no listings and no filters', (
      WidgetTester tester,
    ) async {
      final mockClient = MockClient((request) async {
        if (request.url.toString().contains('health')) {
    ...
    testWidgets('Clears filters via empty state action', (
      WidgetTester tester,
    ) async {
      final mockClient = MockClient((request) async {
        if (request.url.toString().contains('health')) {
