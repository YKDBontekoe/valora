...
void main() {
  ...
  group('WorkspaceProvider', () {
    test('fetchWorkspaces populates list on success', () async {
      when(mockApiService.get('/api/workspaces')).thenAnswer(
        (_) async => [
          {
            'id': '1',
            'name': 'Test',
    ...
    test('fetchWorkspaces handles error', () async {
      when(
        mockApiService.get('/api/workspaces'),
    ...
    test('createWorkspace adds new workspace', () async {
      when(mockApiService.post('/api/workspaces', any)).thenAnswer(
        (_) async => {
          'id': '2',
          'name': 'New',
          'ownerId': 'user1',
    ...
    test('selectWorkspace fetches details', () async {
      when(mockApiService.get('/api/workspaces/1')).thenAnswer(
        (_) async => {
          'id': '1',
          'name': 'WS',
          'ownerId': 'user1',
      ...
      when(
        mockApiService.get('/api/workspaces/1/members'),
      ).thenAnswer((_) async => []);
      when(
        mockApiService.get('/api/workspaces/1/listings'),
      ).thenAnswer((_) async => []);
      when(
        mockApiService.get('/api/workspaces/1/activity'),
      ).thenAnswer((_) async => []);
    ...
    test('inviteMember calls API and refreshes members', () async {
      ...
      // Setup invite call
      when(
        mockApiService.post('/api/workspaces/1/members', any),
      ).thenAnswer((_) async => {});
      when(mockApiService.get('/api/workspaces/1/members')).thenAnswer(
        (_) async => [
          {
            'id': 'wm1',
            'userId': 'u2',
    ...
    test('saveListing calls API and refreshes listings', () async {
      ...
      // Setup save call
      when(
        mockApiService.post('/api/workspaces/1/listings', any),
      ).thenAnswer((_) async => {});
      when(mockApiService.get('/api/workspaces/1/listings')).thenAnswer(
        (_) async => [
          {
            'id': 'sl1',
            'listingId': 'l1',
    ...
    test('addComment calls API', () async {
      ...
      when(
        mockApiService.post('/api/workspaces/1/listings/sl1/comments', any),
      ).thenAnswer((_) async => {});

      await provider.addComment('sl1', 'content', null);

      verify(
        mockApiService.post('/api/workspaces/1/listings/sl1/comments', {
          'content': 'content',
          'parentId': null,
        }),
      ).called(1);
    ...
    test('fetchComments returns list', () async {
      ...
      when(
        mockApiService.get('/api/workspaces/1/listings/sl1/comments'),
      ).thenAnswer(
        (_) async => [
          {
            'id': 'c1',
