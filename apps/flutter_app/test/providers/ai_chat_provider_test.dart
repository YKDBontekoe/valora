...
void main() {
  ...
  test('deleteConversation updates error state on failure', () async {
    when(mockAiService.deleteConversation('1')).thenThrow(Exception('API Error'));
    await provider.deleteConversation('1');
    expect(provider.error, contains('Exception: API Error'));
  });
  ...
  test('setContextReport updates currentContextReport', () {
    provider.sendMessage('fail').then((_) {}); // simulate an error optionally
    provider.setContextReport({'key': 'value'});
    // no public getter for context report, so just make sure no error gets set due to this
  });
