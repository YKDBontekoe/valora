...
const ApiLatencyCard = ({ health, loading, isStale }: ApiLatencyCardProps) => {
  const latencyStatus = health ? getLatencyStatus(health.apiLatencyP95) : null;
      ...
      <AnimatePresence mode="wait">
        {loading && !health && !isStale ? (
          ...
          >
            ...
            {latencyStatus && (
              <span className={`text-[10px] font-black px-3 py-1 rounded-full uppercase tracking-wider border ${latencyStatus.color}`}>
                {latencyStatus.label}
              </span>
            )}
          </motion.div>
